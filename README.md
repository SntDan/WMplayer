...
It uses mpv for playback, reads metadata with mutagen, and supports common lossless and lossy audio formats, including MP3, FLAC, WAV, ALAC, APE, OGG, Opus, and DSD.
...
![Queue view](assets/screenshot.png)
...
| mutagen | >= 1.47 |
| Pillow | >= 10.0 |
| mpv executable | Required on `PATH` or bundled with the app |
...
|-- core/
|   |-- audio_engine.py      # mpv IPC playback engine
...
For Windows releases, keep `mpv.exe` available on `PATH` or bundle the mpv runtime with PyInstaller.
...
播放器基于 mpv 播放音频，使用 mutagen 读取元数据，支持 MP3、FLAC、WAV、ALAC、APE、OGG、Opus、DSD 等常见有损和无损音频格式。
...
![播放队列界面](assets/screenshot.png)

### 搜索

![搜索界面](assets/search.png)

### 歌词

![歌词界面](assets/lyrics.png)
...
| mutagen | >= 1.47 |
| Pillow | >= 10.0 |
| mpv executable | 需要位于 `PATH` 中，或随程序一起打包 |
...
发布 Windows 版本时，需要让 `mpv.exe` 位于 `PATH` 中，或在 PyInstaller 打包时一起带上 mpv 运行时。
